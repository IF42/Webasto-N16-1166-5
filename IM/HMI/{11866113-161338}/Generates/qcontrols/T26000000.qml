import QtQuick 2.0
IGuiTemplate
{
	id: q637534208
	objId: 637534208
	x: 0
	y: 0
	width: 800
	height: 480
	IGuiQmlRectangle
	{
		id: q671088721
		objId: 671088721
		x: 0
		y: 0
		width: 800
		height: 60
		qm_BorderWidth: 0
		qm_TextColor: "#ff181c31"
		qm_FillColor: "#ff949694"
	}
	IGuiQmlRectangle
	{
		id: q671088720
		objId: 671088720
		x: 600
		y: 0
		width: 200
		height: 60
		qm_BorderWidth: 1
		qm_TextColor: "#ff181c31"
		qm_Transparent : true 
	}
	IGuiQmlRectangle
	{
		id: q671088719
		objId: 671088719
		x: 0
		y: 60
		width: 800
		height: 29
		qm_BorderWidth: 1
		qm_TextColor: "#ff9c9aa5"
		qm_FillColor: "#fff7f3f7"
	}
	IGuiQmlRectangle
	{
		id: q671088718
		objId: 671088718
		x: 222
		y: 0
		width: 380
		height: 60
		qm_BorderWidth: 1
		qm_TextColor: "#ff181c31"
		qm_Transparent : true 
	}
	IGuiAlarmView
	{
		id: q402653185
		objId: 402653185
		x: 0
		y: 60
		width: 800
		height: 29
		qm_FillRectWidth: 800
		qm_FillRectHeight: 29
		qm_BorderCornerRadius: 0
		qm_BorderWidth: 0
		qm_BorderColor: "#ff636973"
		qm_ImageID: 46
		qm_TileTop: 0
		qm_TileBottom: 0
		qm_TileRight: 0
		qm_TileLeft: 0
		qm_FillColor: "#ffff0000"
		IGuiListCtrl
		{
			id: qu402653185
			objectName: "qu402653185"
			x: 0
			y: 0
			width: 800
			height: 29
			totalColumnWidth: 798
			qm_leftImageID: 47
			qm_leftTileTop: 7
			qm_leftTileBottom: 7
			qm_leftTileRight: 0
			qm_leftTileLeft: 0
			qm_middleImageID: 48
			qm_middleTileTop: 0
			qm_middleTileBottom: 0
			qm_middleTileRight: 0
			qm_middleTileLeft: 1
			qm_rightImageID: 49
			qm_rightTileTop: 0
			qm_rightTileBottom: 0
			qm_rightTileRight: 0
			qm_rightTileLeft: 1
			qm_leftBorderCornerRadius: 0
			qm_tableBackColor: "#ffff0000"
			qm_tableSelectBackColor: "#ffff0000"
			qm_tableAlternateBackColor: "#ffff0000"
			qm_tableHeaderBackColor: "#ff84868c"
			qm_tableTextColor: "#ff000000"
			qm_tableSelectTextColor: "#ff000000"
			qm_tableAlternateTextColor: "#ff000000"
			qm_tableMarginLeft: 2
			qm_tableMarginRight: 1
			qm_tableMarginBottom: 1
			qm_tableMarginTop: 1
			qm_tableHeaderTextColor: "#ffffffff"
			qm_tableHeaderValueVarTextAlignmentHorizontal: Text.AlignLeft
			qm_tableHeaderValueVarTextAlignmentVertical: Text.AlignVCenter
			qm_tableHeaderValueVarTextOrientation: 0
			qm_tableHeaderMarginLeft: 2
			qm_tableHeaderMarginRight: 0
			qm_tableHeaderMarginBottom: 0
			qm_tableHeaderMarginTop: 0
			qm_gridLineStyle: 0
			qm_gridLineWidth: 0
			qm_gridLineColor: "#ffffffff"
			qm_noOfColumns: 1
			qm_tableRowHeight: 25
			qm_tableHeaderHeight: 14
			qm_hasHeader: false
			qm_hasGridLines: false
			qm_hasBorder: false
			qm_hasDisplayFocusLine: true
			qm_hasVerticalScrolling: false
			qm_hasVerticalScrollBar: false
			qm_hasHorizontalScrollBar: false
			qm_hasColumnOrdering: false
			qm_hasHighLightFullRow: true
			qm_hasVerUpDownPresent: false
			qm_hasVerPgUpDownPresent: false
			qm_hasHighlight: true
			qm_hasUpDownAsPageUpDown: false
			qm_hasLongAlarmButton: true
			qm_hasExtraPixelForLineHeight: false
			qm_hasRowEditable: false
			qm_hasRowJustification: false
			qm_hasRowJustificationBottom: false
			qm_linesPerRow: 1
			IGuiListColumnView
			{
				id: qc118000001
				colIndex: 0
				x: 0
				y: 0
				width: 798
				height: -3
				columnType: 0
				qm_columnValueVarTextAlignmentHorizontal: Text.AlignLeft
				qm_columnValueVarTextAlignmentVertical: Text.AlignVCenter
			}
		}
	}
	IGuiIOField
	{
		id: q33554479
		objId: 33554479
		x: 613
		y: 14
		width: 177
		height: 32
		qm_FillRectWidth: 177
		qm_FillRectHeight: 32
		qm_FontSize: 13
		qm_FontFamilyName: "Tahoma"
		qm_FontBold: true
		qm_BorderCornerRadius: 3
		qm_BorderWidth: 0
		qm_BorderColor: "#ff424952"
		qm_ImageID: 31
		qm_TileTop: 0
		qm_TileBottom: 0
		qm_TileRight: 0
		qm_TileLeft: 0
		qm_Transparent : true 
		qm_FocusWidth: 0
		qm_FocusColor: "#00000000"
		qm_TextColor: "#ff31344a"
		qm_ValueVarTextAlignmentHorizontal: Text.AlignLeft
		qm_ValueVarTextAlignmentVertical: Text.AlignVCenter
		qm_ValueVarTextOrientation: 0
		qm_MarginLeft: 3
		qm_MarginRight: 2
		qm_MarginBottom: 2
		qm_MarginTop: 2
		qm_passwordMode: false
		qm_FontRendringTpe:1
	}
}
